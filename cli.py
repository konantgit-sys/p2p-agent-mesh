...
def main():
    parser = argparse.ArgumentParser(
        description="P2P Agent Mesh — запуск mesh-агента"
    )
    parser.add_argument(
        "--port", type=int, default=0,
        help="TCP port для mesh (0 = random, default)"
    )
    parser.add_argument(
        "--agent", type=str, default="cli_agent",
        help="Имя агента (default: cli_agent)"
    )
    parser.add_argument(
        "--capability", type=str, default="echo",
        help="Capability (default: echo). Можно указать несколько через запятую"
    )
    parser.add_argument(
        "--relay-host", type=str, default=None,
        help="Relay host для NAT traversal"
    )
    parser.add_argument(
        "--relay-port", type=int, default=0,
        help="Relay port"
    )
    parser.add_argument(
        "--listen", action="store_true",
        help="Режим слушателя (подписка на capability)"
    )
    ...
    async def run():
        ...
        print(f"[cli] Listening on port: {agent.transport.port if hasattr(agent, 'transport') else 'N/A'}")
